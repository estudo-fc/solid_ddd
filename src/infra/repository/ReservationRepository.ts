...
import ReservationFactory from "../../application/ReservationFactory";

export interface ReservationRepository {
    hasActiveResevartions(roomId: string, checkinDate: Date, checkoutDate: Date): Promise<boolean>,
...
export class ReservationRepositoryDatabase implements ReservationRepository {
    async hasActiveResevartions(roomId: string, checkinDate: Date, checkoutDate: Date): Promise<boolean> {
    ...
    async updateAllReservationStatus(status: string) {
        const connection = pgp()("postgres://maruko:Postgres2022!@localhost:5432/branas");
